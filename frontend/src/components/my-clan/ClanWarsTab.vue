...
async function loadAvailableClans() {
  ...
  try {
    ...
    // исключаем свой клан
    availableClans.value = data.data.filter(c => c.id !== props.clan.id && !c.is_banned)
  } finally {
      ...
      <section class="section">
        ...
        <div v-else class="list">
          ...
          >
            ...
            <div v-else-if="w.status === 'accepted'" class="war__actions">
              <button
                  class="btn-result"
                  :disabled="processing === w.id"
                  @click="openResult(w)"
      ...
      <section class="section">
        ...
        <div v-else class="list">
          ...
          >
            ...
            <div v-if="w.status === 'accepted'" class="war__actions">
              <button
                  class="btn-result"
                  :disabled="processing === w.id"
                  @click="openResult(w)"
            ...
            <div v-if="w.status === 'completed'" class="war__result">
              ...
              <span v-if="w.winner_clan_id === w.challenger_clan_id" class="win">
                                🏆 Победа
                            </span>
          ...
          <div class="result-teams">
            <div class="result-team">
              <div class="team-label">
                {{ resultWar?.challenger_clan_id === props.clan.id ? 'Ваш клан' : 'Соперник' }}
              </div>
              <div class="team-name">
            ...
            <div class="result-team">
              <div class="team-label">
                {{ resultWar?.opponent_clan_id === props.clan.id ? 'Ваш клан' : 'Соперник' }}
              </div>
              <div class="team-name">
...
.btn-accept, .btn-decline, .btn-result {
  flex: 1;
  padding: 10px 14px;
  border-radius: 9px;
  font-size: 13px;
...
.btn-accept:disabled, .btn-decline:disabled, .btn-result:disabled {
  opacity: 0.5;
  cursor: not-allowed;
}
...
.score { font-size: 18px; font-weight: 900; color: var(--text); letter-spacing: 2px; }
.score b { color: var(--accent-light); }

.win { color: #4ade80; font-weight: 800; font-size: 12px; text-transform: uppercase; }
.loss { color: #f87171; font-weight: 800; font-size: 12px; text-transform: uppercase; }
